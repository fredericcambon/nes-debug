/**
   Modified table from blessed-contrib
*/
...
function Table(options) {
  ...
  // Clicking on any row should set focus to this table
  this.rows.on("select", i => {
    this.rows.focus();
  });

  this.rows.on("keypress", function(ch, key) {
    if (options.vi && key.name == "e") {
      if (typeof self.options.edit !== "function") {
        return null;
      }
      return self.options.edit();
    }
  });
